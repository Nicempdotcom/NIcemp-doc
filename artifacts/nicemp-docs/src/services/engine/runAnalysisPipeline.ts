...
/**
 * runAnalysisPipelineFromFiles
 *
 * Core analysis pipeline that operates on an already-built ScannedFile[] list.
 * Called by both the ZIP path (after extraction) and the GitHub path (after API
 * download). No caller needs to know where the files came from.
 *
 * Progress percentages start at 62 so the ZIP/GitHub download phase (0–60) and
 * this phase share a clean 0–100 range when combined in the UI.
 */
export async function runAnalysisPipelineFromFiles(
  scanned:     ScannedFile[],
  projectName: string,
  { onProgress, isCancelled }: RunPipelineCallbacks,
  /** Starting progress percentage (default 62 to match ZIP extraction phase). */
  startPct = 62,
): Promise<ProjectMap> {
  const counts: LiveCounts = { ...EMPTY_COUNTS };
  counts.filesTotal   = scanned.length;
  counts.filesScanned = scanned.length;

  // Populate quick counts from scanned list
  for (const f of scanned) {
    const cat = quickCat(f.path, f.isBinary);
    if (cat) counts[cat]++;
  }

  const checkCancelled = () => { if (isCancelled()) throw new AnalysisCancelledError(); };

  // ── Categorize + build tree ───────────────────────────────────────────────
  onProgress(startPct, 'Classificando arquivos por tipo...', { ...counts });
  await tick();
  ...
  checkCancelled();
  onProgress(startPct + 3, 'Identificando páginas, componentes e hooks…', { ...counts });
  await tick();

  // ── Dependency map ────────────────────────────────────────────────────────
  onProgress(startPct + 6, 'Construindo mapa de dependências...', { ...counts });
  ...
  // ── Technology stack ──────────────────────────────────────────────────────
  onProgress(startPct + 12, 'Detectando tecnologias e frameworks...', { ...counts });
  ...
  // ── Interactions ──────────────────────────────────────────────────────────
  onProgress(startPct + 17, 'Analisando cliques e interações...', { ...counts });
  ...
  onProgress(startPct + 21, 'Gerando descrições dos arquivos...', { ...counts });
  ...
  // ── Assemble stats ────────────────────────────────────────────────────────
  onProgress(startPct + 31, 'Gerando mapa do projeto...', { ...counts });
  ...
  for (const f of categorized) {
    ...
    // Infer directory count from paths
  ...
  const rootName = (rootPkg?.name && rootPkg.name !== '(unnamed)')
    ? rootPkg.name
    : (dependencies.packages[0]?.name ?? projectName.replace(/\.zip$/i, ''));

  const stats = {
    totalFiles:     scanned.length,
    totalDirs:      dirPaths.size,
  ...
  onProgress(startPct + 33, 'Detectando integrações externas...', { ...counts });
 ...
 * runAnalysisPipeline
 *
 * Full ZIP → ScannedFile[] → ProjectMap pipeline.
 * Unchanged entry point used by the Worker and the main-thread fallback.
 */
export async function runAnalysisPipeline(
  ...
  // ── 3–N. Analysis phases (shared with GitHub path) ───────────────────────
  return runAnalysisPipelineFromFiles(
    scanned,
    fileName,
